...
void InitChorusUI(Plugin* pPlugin, IGraphics* pGraphics, std::vector<IControl*> controls, const std::vector<int>& params, const std::vector<char*>& paramNames, const bool reset)
{
  ...
  for (int i{ 0 }; i < TABLITSA2_EFFECT_PARAMS; ++i)
...
void InitCoefModUI(Plugin* pPlugin, IGraphics* pGraphics, std::vector<IControl*> controls, const std::vector<int>& params, const std::vector<char*>& paramNames, const bool reset)
{
  ...
  dynamic_cast<Tablitsa2IVModKnobControl*>(controls[0])->EnableModulation(true);
...
{
  dynamic_cast<Tablitsa2*>(pPlugin)->SetDelayTempoSync(isTempoSync);
  if (isTempoSync)
  {
  ...
  {
    const IParam* oldParams[]{ pPlugin->GetParam(params[0]), pPlugin->GetParam(params[1]) };
    pPlugin->GetParam(params[0])->InitDouble(oldParams[0]->GetName(), reset ? 100. : oldParams[0]->Value(), 1., TABLITSA2_MAX_EFFECT_DELAY_MS, 1., "ms", IParam::kFlagsNone, "Effect", IParam::ShapePowCurve(3.));
    pPlugin->GetParam(params[1])->InitDouble(oldParams[1]->GetName(), reset ? 100. : oldParams[1]->Value(), 1., TABLITSA2_MAX_EFFECT_DELAY_MS, 1., "ms", IParam::kFlagsNone, "Effect", IParam::ShapePowCurve(3.));
  }
...
void InitDelayUI(Plugin* pPlugin, IGraphics* pGraphics, std::vector<IControl*> controls, const std::vector<int>& params, const std::vector<char*>& paramNames, const bool reset)
{
  ...
  {
    pPlugin->GetParam(params[0])->InitDouble(paramNames[0], 100., 1., TABLITSA2_MAX_DELAY_MS, 1., "ms", IParam::kFlagsNone, "Effect", IParam::ShapePowCurve(3.));
    pPlugin->GetParam(params[1])->InitDouble(paramNames[1], 100., 1., TABLITSA2_MAX_DELAY_MS, 1., "ms", IParam::kFlagsNone, "Effect", IParam::ShapePowCurve(3.));
  }
  pPlugin->GetParam(params[2])->InitDouble(paramNames[2], 0., 0., 1., 0.01);
  pPlugin->GetParam(params[3])->InitDouble(paramNames[3], 0., 0., 1., 0.01);
  // Reload original values if not resetting
  ...
  dynamic_cast<IVKnobControl*>(controls[2])->SetLabelStr("Feedback");
  dynamic_cast<IVKnobControl*>(controls[3])->SetLabelStr("Mix");
  dynamic_cast<Tablitsa2VToggleButton*>(controls[4])->SetText("Time", "Tempo");
  dynamic_cast<Tablitsa2VToggleButton*>(controls[5])->SetText("Stereo", "Mono");
...
void InitDistortionUI(Plugin* pPlugin, IGraphics* pGraphics, std::vector<IControl*> controls, const std::vector<int>& params, const std::vector<char*>& paramNames, const bool reset)
{
  ...
  pPlugin->GetParam(params[2])->InitDouble(paramNames[2], 0., TABLITSA2_DISTORTION_FREQ_LOW, TABLITSA2_DISTORTION_FREQ_HIGH, 0.01, "", 0, "", IParam::ShapeExp());
  pPlugin->GetParam(params[3])->InitDouble(paramNames[3], 0., 0., 1., 0.01);
  // Reload original values if not resetting
  if (!reset)
  {
...
void InitSampleAndHoldUI(Plugin* pPlugin, IGraphics* pGraphics, std::vector<IControl*> controls, const std::vector<int>& params, const std::vector<char*>& paramNames, const bool reset)
{
  ...
  pPlugin->GetParam(params[0])->InitDouble(paramNames[0], 10., TABLITSA2_SAH_MIN_MS, TABLITSA2_SAH_MAX_MS, 0.01, "ms", IParam::kFlagsNone, "Effect", IParam::ShapePowCurve(3.));
...
void InitWaveFolderUI(Plugin* pPlugin, IGraphics* pGraphics, std::vector<IControl*> controls, const std::vector<int>& params, const std::vector<char*>& paramNames, const bool reset)
{
  ...
  dynamic_cast<IVKnobControl*>(controls[3])->SetLabelStr("Mix");
  dynamic_cast<Tablitsa2VToggleButton*>(controls[4])->SetText("Stat. Thr.", "Peak Trk");
  // Toggle buttons
...
void SwapMasterEffectsUI(int effectSlot, IControl* pEffectsList, IGraphics* pGraphics, Plugin* pPlugin, const bool reset)
{
  ...
  for (auto* knob : allKnobs)
    knob->SetDirty(true);
  dynamic_cast<Tablitsa2*>(pPlugin)->RefreshEffectBankControl();
}

void SwapMasterEffectsUI(IControl* pEffectsList, IGraphics* pGraphics, Plugin* pPlugin, const bool reset)
{
  int effectSlot = static_cast<int>(pGraphics->GetControlWithTag(kCtrlTagMasterEffectsSwitch)->GetValue() * (TABLITSA2_MAX_MASTER_EFFECTS - 1)); // ID number of the slot into which the effect will be inserted
  // Save the ID number of newly inserted effect in the pPluginin class effects bank list:
  int effectIdx = dynamic_cast<DropdownListControl*>(pEffectsList)->GetCurrentIndex(); // ID number of the effect
  dynamic_cast<Tablitsa2*>(pPlugin)->SetMasterFXSlot(effectSlot, static_cast<EMasterEffectTypes>(effectIdx));
...
void SwapVoiceEffectsUI(int effectSlot, IControl* pEffectsList, IGraphics* pGraphics, Plugin* pPlugin, const bool reset)
{
  ...
  // Clear labels
  dynamic_cast<Tablitsa2VToggleButton*>(toggle1)->SetText("");
  dynamic_cast<Tablitsa2VToggleButton*>(toggle2)->SetText("");
...
void SwapVoiceEffectsUI(IControl* pEffectsList, IGraphics* pGraphics, Plugin* pPlugin, const bool reset)
{
  ...
  int effectIdx = dynamic_cast<DropdownListControl*>(pEffectsList)->GetCurrentIndex(); // ID number of the effect
  // Save the ID number of newly inserted effect in the pPluginin class effects bank list:
  int effectSlot = static_cast<int>(pGraphics->GetControlWithTag(kCtrlTagVoiceEffectsSwitch)->GetValue() * (TABLITSA2_MAX_VOICE_EFFECTS - 1)); // ID number of the slot into which the effect will be inserted
  dynamic_cast<Tablitsa2*>(pPlugin)->SetVoiceFXSlot(effectSlot, static_cast<EVoiceEffectTypes>(effectIdx));
