...
public:
  ...
  */
  ...
  inline double AddModulation_Vector(double initVal, double* modValues) const
  {
    ...
    Vec4d modValuesV;
    auto i{ 0 };
    for (; i < (NMods & vectorsize); i+=4)
    {
      modDepths.load(mModDepths);
      modValuesV.load(modValues);
      modulation += horizontal_add(modDepths * modValuesV);
    }
    modDepths.load_partial(NMods - i, mModDepths + i);
    modDepths.cutoff(NMods - i);
    modValuesV.load_partial(NMods - i, modValues + i);
...
protected:
  double mModValues[NMods]{ 0. };
  double mStaticModulation{};

  double mInitial{ 0. };
  double mModDepths[NMods]{ 0. };
...
template<typename T, int NParams, int NMods, int VectorSize = 4>
class ModulatedParameterList
{
public:
  ModulatedParameterList(std::initializer_list<ParameterModulator<NMods>*> params) : mParams(params), mParamBlocks(NParams / 4)
  {}
  ...
  */
  ...
  ParameterModulator<NMods>& operator[](int idx)
  {
    return *(mParams[idx]);
  }

private:
  std::vector<ParameterModulator<NMods>*> mParams;
...
*/
template<typename T, class EnvType, class LFOType, class SequencerType, int NMods>
class ModulatorList
{
public:
  ModulatorList() {}
  ...
  void AddModulator(EnvType* env, std::vector<ParameterModulator<NMods>*>& metaParams)
  {
    AddModulator(env);
  ...
  void AddModulator(LFOType* lfo, std::vector<ParameterModulator<NMods>*>& metaParams)
  {
    AddModulator(lfo);
    mMetaParams.push_back(metaParams);
  }
  ...
  void AddModulator(SequencerType* seq, std::vector<ParameterModulator<NMods>*>& metaParams)
  {
    AddModulator(seq);
    mMetaParams.push_back(metaParams);
  }

  /* Adds MIDI control ramps as a modulator.
  */
  void AddControls(T* controlRamp)
  {
    mModulators.Set(mModPtrs.size(), controlRamp);
  }
  ...
  inline void MetaProcessBlock_Fast(T** inputs, int nFrames)
  {
    ...
    // Loop through the first value of all the modulators, setting meta-parameters and processing the first sample of the buffer
    for (auto m{ 0 }; m < mModPtrs.size(); ++m)
    {
...
private:
  ...
  std::vector<std::vector<ParameterModulator<NMods>*>> mMetaParams;
  int mNumMods{ 0 };
  int mNumEnvs{ 0 };
  int mNumLFOs{ 0 };
  int mNumSeqs{ 0 };
  int mNumControlRamps{ 0 };
