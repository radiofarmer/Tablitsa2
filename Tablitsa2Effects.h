...
#define TABLITSA2_MAX_DELAY_SAMP (int)524288
#define TABLITSA2_MAX_DELAY_MS 524288. / 48000. * 1000.
#define TABLITSA2_CHORUS_VOICES 4
...
{
private:
  static constexpr T AmpNorm = 1. / TABLITSA2_CHORUS_VOICES;
  ...
  void ResetVoices()
  {
    for (int i{ 0 }; i < TABLITSA2_CHORUS_VOICES; ++i)
    {
      mVoices[i].pitch_offset = mDepth * ((i + 1) / TABLITSA2_CHORUS_VOICES - 0.5);
      T pan = mSpread * ((i + 1) / TABLITSA2_CHORUS_VOICES - 0.5) + 0.5;
      mVoices[i].pan_l = std::sqrt(pan);
      mVoices[i].pan_r = std::sqrt(1 - pan);

      mAllpassL[i].SetCutoffAndResonance(0.2 + 0.19 * (i / TABLITSA2_CHORUS_VOICES) * mSpread, 0.5);
      mAllpassR[i].SetCutoffAndResonance(0.2 + 0.19 * (i / TABLITSA2_CHORUS_VOICES) * mSpread, 0.5);
    }
  }
...
protected:
  ...
  ChorusVoice mVoices[TABLITSA2_CHORUS_VOICES];

  DelayLine<4096> mDelayL;
  DelayLine<4096> mDelayR;
  TwoPoleTPTFilter mAllpassL[TABLITSA2_CHORUS_VOICES];
  TwoPoleTPTFilter mAllpassR[TABLITSA2_CHORUS_VOICES];

  FastSinOscillator<T> mRateOsc;
};

template<typename T, int MaxDelay=TABLITSA2_MAX_DELAY_SAMP, class V = Vec4d>
...
public:
  ...
  DelayEffect(double sampleRate, ModMetronome* metronome=nullptr) :
    ...
    mMaxDelayMS(TABLITSA2_MAX_DELAY_MS),
    mMaxDelay(MaxDelay),
  ...
  void CalculateDelaySamples()
  {
    ...
    mDelayLTime = std::min(mDelayLTime, TABLITSA2_MAX_DELAY_SAMP);
    mDelayRTime = std::min(mDelayRTime, TABLITSA2_MAX_DELAY_SAMP);
  }
...
private:
  ...
  DelayLine<TABLITSA2_MAX_DELAY_SAMP>  mDelayL;
  DelayLine<TABLITSA2_MAX_DELAY_SAMP> mDelayR;
