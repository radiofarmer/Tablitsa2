...
if %1 == 1 (echo Making Tablitsa2 Windows DEMO VERSION distribution ...) else (echo Making Tablitsa2 Windows FULL VERSION distribution ...)
...
REM - msbuild Tablitsa2-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild Tablitsa2.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild Tablitsa2.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Tablitsa2.aaxplugin\Contents\Win32\Tablitsa2.aaxplugin --out .\build-win\aax\bin\Tablitsa2.aaxplugin\Contents\Win32\Tablitsa2.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Tablitsa2.aaxplugin\Contents\x64\Tablitsa2.aaxplugin --out .\build-win\aax\bin\Tablitsa2.aaxplugin\Contents\x64\Tablitsa2.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\Tablitsa2.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\Tablitsa2.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Tablitsa2 Installer" ".\installer\Tablitsa2 Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\Tablitsa2 Installer.exe" ".\installer\Tablitsa2 Demo Installer.exe"
REM -del ".\installer\Tablitsa2 Installer.exe"
REM -)
