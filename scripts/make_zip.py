...
def  main():
  ...
  installer = "\installer\Tablitsa2 Installer.exe"
   
  if demo:
    installer = "\installer\Tablitsa2 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Tablitsa2 manual.pdf" 
  ]
  ...
  ZIPNAME = "Tablitsa2-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Tablitsa2-v" + FULLVERSIONSTR + "-win-demo.zip"
