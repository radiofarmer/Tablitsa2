...
def ReadParams(filename="Tablitsa2.h"):
  p = re.compile(r"\s+(.+?)[\s|,]")
  ...
  end_line = start_line + all_text[start_line:].index("};\n")
  return [p.findall(l)[0] for l in all_text[start_line:end_line] if p.match(l) and "kNumParams" not in l]
